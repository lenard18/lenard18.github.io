...
@media (max-width: 1100px) {
  ...
  .cuadro-metraje-content { flex-direction: column; }
}
...
@media (max-width: 768px) {
  ...
  .factura { padding: 20px; }
  .factura-header { flex-direction: column; }
  .tabla-factura { font-size: 0.85em; }
  .btn-group { grid-template-columns: 1fr; }
}

@media print {
  body { background: white; }
  .panel:not(.factura) { display: none; }
  .btn-imprimir, .btn-whatsapp { display: none; }
}
</style>
</head>
<body>
...
<div class="container">
  ...
  <div class="panel factura">
    ...
    <div id="cuadroMedidas" class="cuadro-metraje" style="display: none;">
      <div class="cuadro-metraje-content">
        ...
        <div class="cuadro-metraje-right">
          <div style="font-size: 0.9em; opacity: 0.9; margin-bottom: 5px;">METROS CUADRADOS</div>
    ...
    <div class="cliente-info">
      ...
      <label style="color: #666; font-size: 0.85em;">Nombre:</label>
      <input type="text" id="clienteNombre" placeholder="Nombre completo del cliente">
      <label style="color: #666; font-size: 0.85em;">Teléfono:</label>
      <input type="text" id="clienteTelefono" placeholder="Número de contacto">
      <label style="color: #666; font-size: 0.85em;">Dirección:</label>
...
function redondearTornillos(cantidad) {
  ...
  if (resto === 0) return cantidad;
  return resto <= 50 ? cantidad - resto + 50 : cantidad - resto + 100;
}
...
function generarFactura() {
  ...
  if (document.getElementById('tab-dimensiones').classList.contains('active') && areaTotal > 0) {
    ...
    infoMedidas.innerHTML = `<span style="font-size: 0.75em;">Ancho: ${ancho.toFixed(2)}m × Largo: ${largo.toFixed(2)}m</span>`;
  } else {
  ...
  items.forEach(item => {
    ...
    tr.innerHTML = `
      ...
      <td style="text-align: center;"><input type="number" value="${item.cant}" data-key="${item.key}" data-type="cant" oninput="recalcularTotal()"></td>
      <td style="text-align: right;"><input type="number" value="${item.precio}" data-key="${item.key}" data-type="precio" oninput="recalcularTotal()"></td>
      <td class="subtotal" data-key="${item.key}">$${formatearPrecio(subtotal)}</td>
    `;
...
function recalcularTotal() {
  ...
  filas.forEach(fila => {
    const inputCant = fila.querySelector('input[data-type="cant"]');
    const inputPrecio = fila.querySelector('input[data-type="precio"]');
    const celdaSubtotal = fila.querySelector('.subtotal');
    
    if (inputCant && inputPrecio && celdaSubtotal) {
  ...
  document.getElementById('totalFactura').textContent = '$' + formatearPrecio(total);
}
...
function enviarWhatsApp() {
  ...
  let mensaje = `*COTIZACIÓN PVC PRO*\n\n`;
  mensaje += `*Cliente:* ${cliente}\n`;
  if (telefono) mensaje += `*Teléfono:* ${telefono}\n`;
  mensaje += `\n*DETALLE:*\n`;
  
  const filas = document.getElementById('tablaItems').querySelectorAll('tr');
  filas.forEach(fila => {
    const desc = fila.querySelector('.item-desc');
    const cant = fila.querySelector('input[data-type="cant"]');
    const precio = fila.querySelector('input[data-type="precio"]');
    const subtotal = fila.querySelector('.subtotal');
    
    if (desc && cant && precio && subtotal) {
      mensaje += `\n${desc.textContent}\n`;
      mensaje += `Cant: ${cant.value} | Precio: $${formatearPrecio(parseFloat(precio.value))}\n`;
      mensaje += `Subtotal: ${subtotal.textContent}\n`;
    }
  });
  
  mensaje += `\n*TOTAL: ${total}*\n\n`;
  mensaje += `Validez: 15 días`;
